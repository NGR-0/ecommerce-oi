...
var (
	...
	ErrUserIdIsNotValid   = errors.New("this user not valid")
...
func BuyItemFromCart(ctx context.Context, userCollection *mongo.Collection, userID string) error {
	...
	grouping := bson.D{{Key: "$group", Value: bson.D{primitive.E{Key: "_id", Value: "$_id"}, {Key: "total", Value: bson.D{primitive.E{
		Key:   "$sum",
	...
	update2 := bson.M{"$push": bson.M{"orders.$[].order_list": bson.M{"$each": getcartitem.UsersCart}}}
...
func InstantBuyer(ctx context.Context, prodCollection, userCollection *mongo.Collection, productID primitive.ObjectID, userID string) error {
	...
	orders_detail.Price = product_detail.Price
